...
class CurrenciesViewModel @Inject constructor(
    ...
    fun onStart() {
        currencyRatesUiModelDisposer =
            getCurrencyRatesUiModel()
    ...
    private fun createCurrencyRatesUiModel(baseCurrencyChanged: Boolean) =
        getRatesWithBase()
            ...
            .onErrorResumeNext {
                Single.just(
                    CurrencyRatesUiModel(
                        CurrencyRatesUiModel.Status.ERROR,
                        false,
                        listOf(currentBaseCurrency.value!!)
                    )
                )
            }

    private fun getRatesWithBase() =
        currenciesRepository.getRates(currentBaseCurrency.value!!.code)
            .map { currencyRates ->
    ...
    data class CurrencyRatesUiModel(
        val status: Status,
        val baseCurrencyChanged: Boolean,
